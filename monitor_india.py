...
LOG_FILE = os.path.join(os.path.dirname(__file__), "india_signals.log")
...
def print_header():
    print(f"\n{BOLD}{CYAN}{'='*65}")
    print(f"   INDIAN MARKET MONITOR  |  NIFTY + BANKNIFTY  |  v2.0")
    print(f"   {datetime.now().strftime('%Y-%m-%d %H:%M:%S')}")
    print(f"   NSE Hours: 9:15 AM - 3:30 PM IST")
    print(f"{'='*65}{RESET}\n")
...
def print_signal(sig):
    ...
    print(f"  Risk/unit:      {sig['risk_per_unit']} pts")
    print(f"  Suggested Qty:  {sig['qty_suggestion']} units (1% of 1L capital)")

    print(f"\n  {BOLD}{WHITE}---- TARGETS ----{RESET}")
    print(f"  Target 1:       {sig['target_1']}  |  R:R = {sig['rr_t1']}  |  Profit: {sig['reward_t1']} pts")
    print(f"  Target 2:       {sig['target_2']}  |  R:R = {sig['rr_t2']}  |  Profit: {sig['reward_t2']} pts")

    print(f"\n  {BOLD}{WHITE}---- SIGNAL STRENGTH ----{RESET}")
    print(f"  Score:          {sig['score']}/{sig['max_score']}")
    print(f"  RSI:            {sig['rsi']}")
    if sig['adx']:
        print(f"  ADX:            {sig['adx']} ({'Strong trend' if sig['trend_strong'] else 'Weak trend'})")
    ...
    print(f"\n  {BOLD}{WHITE}---- HOW TO TRADE (Zerodha Kite) ----{RESET}")
    if action == "BUY":
        print(f"    1. Open Kite > search {sig['name']}")
        print(f"    2. BUY at {sig['price']} or market price")
        print(f"    3. Set SL order at {sig['stop_loss']}")
        print(f"    4. Book 50% at Target 1 ({sig['target_1']})")
        print(f"    5. Trail SL to entry, ride to Target 2 ({sig['target_2']})")
        print(f"    6. MAX LOSS: {sig['risk_per_unit']} pts | MAX GAIN: {sig['reward_t2']} pts")
    else:
        print(f"    1. Open Kite > search {sig['name']}")
        print(f"    2. SELL/SHORT at {sig['price']} or market price")
        print(f"    3. Set SL order at {sig['stop_loss']}")
        print(f"    4. Book 50% at Target 1 ({sig['target_1']})")
        print(f"    5. Trail SL to entry, ride to Target 2 ({sig['target_2']})")
        print(f"    6. MAX LOSS: {sig['risk_per_unit']} pts | MAX GAIN: {sig['reward_t2']} pts")
...
def print_overview(summaries):
    for s in summaries:
        ...
        print(f"  Buy Score:      {GREEN}{s['buy_score']:.1f}/12{RESET}")
        print(f"  Sell Score:     {RED}{s['sell_score']:.1f}/12{RESET}")

        if s["buy_reasons"]:
            print(f"  {GREEN}Bullish:{RESET} {', '.join(s['buy_reasons'])}")
        if s["sell_reasons"]:
            print(f"  {RED}Bearish:{RESET} {', '.join(s['sell_reasons'])}")

        needed_buy = max(0, 4 - s["buy_score"])
        needed_sell = max(0, 4 - s["sell_score"])
        if needed_buy <= 1.5:
            print(f"  {BOLD}{GREEN}>>> BUY forming! Need {needed_buy:.1f} more{RESET}")
        elif needed_sell <= 1.5:
            print(f"  {BOLD}{RED}>>> SELL forming! Need {needed_sell:.1f} more{RESET}")
        print()
    ...
    try:
        while True:
            ...
            if signals:
                ...
                print(f"  {BOLD}{YELLOW}*** TRADE SIGNALS ***{RESET}\n")
                for sig in signals:
                    print_signal(sig)
                    log_signal(sig)
            else:
                print(f"  {DIM}No trade signals. Monitoring...{RESET}\n")
        ...
        print(f"\n  {YELLOW}Stopped. Past signals in india_signals.log{RESET}\n")
